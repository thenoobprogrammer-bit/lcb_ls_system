...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (in_array($action, ['add', 'edit'], true)) {
            $equipmentIds = array_map('intval', (array) ($_POST['equipment_ids'] ?? []));
            $summary = [];
            ...
            if ($action === 'add') {
                $stmt = db()->prepare('INSERT INTO packages (package_name, description, equipment_ids, equipment_summary, price, availability_status, approval_status, submitted_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
                ...
                $stmt->bind_param('ssssdssi', $packageName, $description, $idsString, $summaryString, $price, $availability, $approvalStatus, $userId);
                $stmt->execute();
            ...
            if ($action === 'edit') {
                $id = post_int('id');
                $stmt = db()->prepare('UPDATE packages SET package_name = ?, description = ?, equipment_ids = ?, equipment_summary = ?, price = ?, availability_status = ?, approval_status = ? WHERE id = ? AND submitted_by = ?');
                $packageName = post_string('package_name');
                $description = post_string('description');
                $price = post_float('price');
                $availability = post_string('availability_status');
                $stmt->bind_param('ssssdssii', $packageName, $description, $idsString, $summaryString, $price, $availability, $approvalStatus, $id, $userId);
                $stmt->execute();
...
<div class="modal fade" id="addPackageModal" tabindex="-1">
    <div class="modal-dialog modal-lg"><div class="modal-content"><form method="post">
        ...
        <div class="modal-body row g-3">
            ...
            <div class="col-12"><label class="form-label">Included Equipment</label><div class="row g-2"><?php foreach ($inventory as $item): ?><div class="col-md-6"><div class="form-check border rounded-3 p-2"><input class="form-check-input" type="checkbox" name="equipment_ids[]" value="<?= (int) $item['id'] ?>" id="empEqAdd<?= (int) $item['id'] ?>"><label class="form-check-label" for="empEqAdd<?= (int) $item['id'] ?>"><?= e($item['item_name']) ?> (<?= e($item['status']) ?>)</label></div></div><?php endforeach; ?></div></div>
        </div>
...
<?php foreach ($packages as $package): if ((int) $package['submitted_by'] !== $userId) continue; $selectedIds = array_map('intval', array_filter(explode(',', (string) $package['equipment_ids']))); ?>
<div class="modal fade" id="editPackage<?= (int) $package['id'] ?>" tabindex="-1">
    <div class="modal-dialog modal-lg"><div class="modal-content"><form method="post">
        ...
        <div class="modal-body row g-3">
            ...
            <div class="col-12"><label class="form-label">Included Equipment</label><div class="row g-2"><?php foreach ($inventory as $item): ?><div class="col-md-6"><div class="form-check border rounded-3 p-2"><input class="form-check-input" type="checkbox" name="equipment_ids[]" value="<?= (int) $item['id'] ?>" id="empEq<?= (int) $package['id'] ?>_<?= (int) $item['id'] ?>" <?= in_array((int) $item['id'], $selectedIds, true) ? 'checked' : '' ?>><label class="form-check-label" for="empEq<?= (int) $package['id'] ?>_<?= (int) $item['id'] ?>"><?= e($item['item_name']) ?></label></div></div><?php endforeach; ?></div></div>
        </div>
